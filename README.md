...
단일 스레드 이벤트 루프가 왜 빠른지, `ziplist`가 `skiplist`로 전환되는 조건이 무엇인지, `BGSAVE`의 `fork()`가 Copy-On-Write로 일관성을 보장하는 원리, Cache Stampede를 막는 PER 알고리즘까지  
...
| "Hash는 HashMap처럼 쓰면 됩니다" | `ziplist` → `hashtable` 전환 조건, 점진적 rehashing 중에도 읽기/쓰기가 가능한 이유 |
...
| [02. 메모리 관리 — jemalloc과 maxmemory 정책](./redis-internals/02-memory-management.md) | jemalloc 할당기가 메모리 단편화를 줄이는 방식, `mem_allocator_frag_ratio`로 단편화 측정, `maxmemory-policy` 8가지(noeviction/allkeys-lru/volatile-lru 등) 내부 동작과 서비스 특성별 선택 기준 |
...
| [03. Hash — ziplist에서 hashtable로, 점진적 rehashing](./data-structures/03-hash-internals.md) | `hash-max-ziplist-entries`/`hash-max-ziplist-value` 임계값에서 ziplist → hashtable 전환 조건, 점진적 rehashing이 두 개의 hashtable을 동시에 유지하면서 읽기/쓰기를 처리하는 방식, rehashing 중 메모리가 두 배가 되지 않는 이유 |
| [04. Set과 Sorted Set — intset / skiplist 내부](./data-structures/04-set-sortedset-internals.md) | Set의 intset이 정수를 압축 저장하고 이진 탐색으로 O(log N)을 보장하는 방식, Sorted Set의 `ziplist` → `skiplist + hashtable` 전환 조건, 스킵 리스트가 `O(log N)` 삽입/삭제를 보장하는 확률적 레이어 구조 |
...
| [03. 메모리 최적화 실전 — 인코딩 임계값 튜닝](./performance-operations/03-memory-optimization.md) | `hash-max-ziplist-entries` / `hash-max-ziplist-value` 등 데이터 구조별 임계값 튜닝이 메모리에 미치는 실제 영향, `MEMORY USAGE key`로 정확한 메모리 측정, `redis-cli --bigkeys`로 대형 키 탐색, `OBJECT FREQ`로 접근 패턴 분석 |
